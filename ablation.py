...
Ablation_nmse = np.empty([7, int(Nv*N*Nf*0.2)])
Combiner_nmse = np.empty([7, int(Nv*N*Nf*0.2)])
...
for SNR in np.arange(0, 31, 5):
    ...
    # # train
    # model2, history2 = train(x_train2, y_train, x_test2, y_test, build_integrator(3,4), 500)
    # model2.save(f'model/ablation{SNR}.h5')
    # for key in history2.history:
    #     history2.history[key] = [float(i) for i in history2.history[key]]
    # with open(f'loss/ablation{SNR}.json', 'w') as f:
    #     json.dump(history2.history, f)
    
    # test
    model2 = models.load_model(f'model/ablation{SNR}.h5')
    y_pred2 = model2.predict(x_test1)
    y_pred2 = y_pred2[:,:,:,:,0] + 1j * y_pred2[:,:,:,:,1] # (N_test, 12, 32, 14)

    y_test = y_test[:,:,:,:,0] + 1j * y_test[:,:,:,:,1] # (N_test, 12, 32, 14)
    y_esti = x_test[:,:,:,:,0] + 1j * x_test[:,:,:,:,1] # (N_test, 12, 32, 14)

    y_pred3 = (x_test1[:,:,:,:,0] + 1j * x_test1[:,:,:,:,1] + \
               x_test1[:,:,:,:,2] + 1j * x_test1[:,:,:,:,3]) / 2 # (N_test, 12, 32, 14)
    
    Wm = 0.5 - 0.01 * SNR
    Wp = 0.5 + 0.01 * SNR
    y_p = x_test1[:,:,:,:,0] + 1j * x_test1[:,:,:,:,1]
    y_m = x_test1[:,:,:,:,2] + 1j * x_test1[:,:,:,:,3]
    y_pred3 = Wm * y_m + Wp * y_p

    Ablation_nmse[int(SNR/5),:] = cal_nmse(y_test, y_pred2)
    Combiner_nmse[int(SNR/5),:] = cal_nmse(y_test, y_pred3)

# calculate nmse and SNR
print('Ablation nmse:', np.round(np.mean(Ablation_nmse, 1), 4))
print('Combiner nmse:', np.round(np.mean(Combiner_nmse, 1), 4))

# save data
np.save('result/Ablation_nmse.npy', Ablation_nmse)
np.save('result/Combiner_nmse.npy', Combiner_nmse)