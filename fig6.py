...
UNet_nmse = np.load('result/UNet_nmse.npy')
...
for i in range(Nv):
    ...
    UNet_nmse1[i,:] = np.mean(UNet_nmse[:, index], axis=-1)
...
# print('UNet nmse:', np.round(UNet_nmse1[:,SNR], 4))
...
UNet_nmse1 = np.array([0.0318, 0.0389, 0.0504, 0.0708, 0.0843, 0.1155])
...
plt.plot(v, ChannelNet_nmse1, color='#FFBB00', marker = 'o', markersize=10, lw=2, label='ChannelNet [26]')
plt.plot(v, cgan_nmse1, color='#7CBB00', lw=2, marker = 's', markersize=10, label='CGAN [29]')
plt.plot(v, UNet_nmse1, color='#00CCCC', lw=2, marker = 'd', markersize=10, label='RadioUNet [58]')
plt.plot(v, LSTM_nmse1, color='#00A1F1', lw=2, marker = 'p', markersize=10, label='DNN+LSTM [27]')
plt.plot(v, RadioMap_nmse1, color='#68217A', lw=2, marker = 'H', markersize=10, label='Radio Map')

plt.xticks(np.arange(5,31,5))
plt.legend(fontsize=font2, loc='lower right', ncol=2)
plt.xlabel('Velocity (m/s)', fontsize=font1)
plt.ylabel('NMSE', fontsize=font1)
...
UNet_nmse1 = np.array([0.0162, 0.0171, 0.0253, 0.039,  0.0529, 0.0848])
...
plt.xticks(np.arange(5,31,5))
plt.legend(fontsize=font2, loc='upper left', ncol=2)
plt.xlabel('Velocity (m/s)', fontsize=font1)
plt.ylabel('NMSE', fontsize=font1)
plt.xticks(fontsize=font1)
plt.yticks(fontsize=font1)
...
UNet_nmse1 = np.array([0.0127, 0.0124, 0.0189, 0.0346, 0.0494, 0.0813])
...
plt.xticks(np.arange(5,31,5))
plt.legend(loc='upper left', fontsize=font2, ncol=2)
plt.xlabel('Velocity (m/s)', fontsize=font1)
plt.ylabel('NMSE', fontsize=font1)
plt.xticks(fontsize=font1)
plt.yticks(fontsize=font1)
